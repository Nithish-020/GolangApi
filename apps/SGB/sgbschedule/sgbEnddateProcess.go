					...
					if lErr2 != nil {
						...
						if len(lBrokerIdArr) != 0 {
							...
							for lBrokerIdx := 0; lBrokerIdx < len(lBrokerIdArr); lBrokerIdx++ {
								// lWg.Add(1)
								// jvsuccessCount, jvfailedCount, ExchangeSuccess, ExchangeFailed, ReverJvSuccess, ReverseJvFailed, lTotalReqCount, lFoJvsuccess, lFoJvFailed, lErr3 := ProcessSgbOrder(lValidSgbArr[lSgbIdx], lBrokerIdArr[lBrokerIdx], r)
								...
								if lErr3 != nil {
									...
									// lsgbIssue.TotalReqCount = lTotalReqCount
									// lsgbIssue.JvSuccesssCount = jvsuccessCount
									// lsgbIssue.JvFailedCount = jvfailedCount
									// lsgbIssue.ExchangeSuccessCount = ExchangeSuccess
									// lsgbIssue.ExchangeFailedCount = ExchangeFailed
									// lsgbIssue.ReverseJvSuccessCount = ReverJvSuccess
									// lsgbIssue.ReverseJvFailedCount = ReverseJvFailed
									// lsgbIssue.FoJvSuccessCount = lFoJvsuccess
									// lsgbIssue.FoJvFailedCount = lFoJvFailed
...
func fetchSgbOrder(pExchange string, BrokerId int, pMasterId int) ([]bsesgb.SgbReqStruct, []JvReqStruct, error) {
	...
	log.Println("lConfigTime", lConfigTime)
	log.Println("lSpecificClient", lSpecificClient)
	...
	if lErr1 != nil {
		...
		if lCurrentTimeUnix > lConfigUnixTime {
			...
			if lErr1 != nil {
				...
				lFinalQueryString := lSqlString + lProcessFlagString + lCondition
